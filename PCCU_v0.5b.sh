...
#  v0.5:
#  + Added Flush DNS.
#  + Added More Clear Bash History Option.
#  + Adding Clear Clipboard Option (not all linux distro successfully works).
#  + Fixed Apt Bug Caused by (apt -y).
#  + Fixed 'Apt Fix' Command.
#
#  What Things Will Be Added (0.5):
...
home(){
	...
	echo -e $__COLOR_LGREEN" PCCU v0.5"
	echo -e $__COLOR_LBLUE"============================================"
  ...
  echo "[L] Clear Clipboard"
	...
	if [ "$CHOICES" != "X" ] || [ "$CHOICES" != "x" ];then
		echo -e $__COLOR_LBLUE"============================================"
		echo -e $__COLOR_YELLOW" PCCU v0.5"
		...
		if [ "$CHOICES" = "L" ] || [ "$CHOICES" = "l" ];then
			echo -e $__COLOR_YELLOW"PCCU: Clearing Clipboard..."$__COLOR_END
			uclip -z
			xclip -i /dev/null
